...
public class DeepLinkIntentReceiver extends ActivityView {
        ...
        if (uri.getPath() != null && uri.getPath().contains("group")) {
            ...
            if (!TextUtils.isEmpty(lastPathSegment)) {
                try {
                    ...
                    Logger.getInstance().v(str2, "aptoide web site: bundle: " + uri2.toString());
            ...
            if (strArrSplit != null && strArrSplit.length == 4) {
                ...
                Logger.getInstance().v(TAG, "aptoide web site: app view: " + strArrSplit[0]);
                return startAppView(strArrSplit[0]);
            }
            if (strArrSplit != null && strArrSplit.length == 3) {
                this.deepLinkAnalytics.websiteFromHomeWebPage();
                Logger.getInstance().v(TAG, "aptoide web site: home: " + strArrSplit[0]);
                return startFromHome();
            }
        }
        return null;
    ...
    private Intent dealWithMarketSchema(String str, Uri uri) {
        ...
        if ("details".equalsIgnoreCase(uri.getHost())) {
            strSubstring = uri.getQueryParameter(DeepLinksKeys.ID);
        ...
        if (mapSplitQuery != null) {
            ...
            if (value != null) {
                try {
                    ...
                    CrashReport.getInstance().log(e3);
                    ShowMessage.asToast(getApplicationContext(), R.string.simple_error_occured + value);
                }
            }
        }
        return null;
    ...
    private Intent parseAptoideInstallUri(String str) {
        ...
        if (aptoideInstall.getAppId() > 0) {
            return startFromAppView(aptoideInstall.getAppId(), aptoideInstall.getPackageName(), aptoideInstall.getOpenType(), aptoideInstall.getOrigin());
        }
        return !TextUtils.isEmpty(aptoideInstall.getUname()) ? startAppView(aptoideInstall.getUname()) : startFromAppview(aptoideInstall.getStoreName(), aptoideInstall.getPackageName(), aptoideInstall.getOpenType(), aptoideInstall.getOrigin());
    }
    ...
    protected void onCreate(Bundle bundle) {
        ...
        if (uri.getHost() != null) {
            if (uri.getHost().contains(cm.aptoide.pt.dataprovider.BuildConfig.APTOIDE_WEB_SERVICES_HOST)) {
                ...
                aptoideAuthUri = startFromPackageName(uri.getQueryParameter(DeepLinksKeys.ID));
    ...
    public Intent startFromPackageName(String str) {
        ...
        if (str != null) {
            try {
                getAppB = GetAppRequest.of(str, ((AptoideApplication) getApplicationContext()).getAccountSettingsBodyInterceptorPoolV7(), ((AptoideApplication) getApplicationContext()).getDefaultClient(), WebService.getDefaultConverter(), ((AptoideApplication) getApplicationContext()).getTokenInvalidator(), ((AptoideApplication) getApplicationContext()).getDefaultSharedPreferences()).observe().W0().b();
        ...
        return (getAppB == null || !getAppB.isOk()) ? startFromSearch(str) : startFromAppView(str);
    }
