...
public class ReactionsView extends LinearLayout {
    ...
    public void setReactionOrientation() {
        ...
        if (i2 == 1) {
            WIDTH_REACTIONS = Math.round(point.x / 5);
            HEIGHT_REACTIONS = Math.round(point.x / 5);
            REACTIONS_PADDING = Math.round((WIDTH_REACTIONS / 100) * 7);
        } else {
            WIDTH_REACTIONS = Math.round((point.x / 2) / 5);
            HEIGHT_REACTIONS = Math.round((point.x / 2) / 5);
            REACTIONS_PADDING = Math.round((WIDTH_REACTIONS / 100) * 7);
        }
    }
