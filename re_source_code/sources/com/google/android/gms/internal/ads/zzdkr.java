...
public final class zzdkr {
    ...
    public static KeyPair b(ECParameterSpec eCParameterSpec) throws GeneralSecurityException {
        KeyPairGenerator keyPairGeneratorB = zzdkx.f15965h.b("EC");
        keyPairGeneratorB.initialize(eCParameterSpec);
    ...
    public static ECPublicKey d(zzdkt zzdktVar, byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        ECParameterSpec eCParameterSpecE = e(zzdktVar);
        ECPoint eCPoint = new ECPoint(new BigInteger(1, bArr), new BigInteger(1, bArr2));
        h(eCPoint, eCParameterSpecE.getCurve());
        return (ECPublicKey) zzdkx.f15966i.b("EC").generatePublic(new ECPublicKeySpec(eCPoint, eCParameterSpecE));
    }
    ...
    static void g(ECPublicKey eCPublicKey) throws GeneralSecurityException {
        h(eCPublicKey.getW(), eCPublicKey.getParams().getCurve());
    }

    static void h(ECPoint eCPoint, EllipticCurve ellipticCurve) throws GeneralSecurityException {
        ...
        if (affineX.signum() == -1 || affineX.compareTo(bigIntegerA) != -1) {
            throw new GeneralSecurityException("x is out of range");
        }
        if (affineY.signum() == -1 || affineY.compareTo(bigIntegerA) != -1) {
    ...
    public static byte[] i(ECPrivateKey eCPrivateKey, ECPoint eCPoint) throws GeneralSecurityException {
        ...
        KeyAgreement keyAgreementB = zzdkx.f15964g.b("ECDH");
        keyAgreementB.init(eCPrivateKey);
        try {
            keyAgreementB.doPhase(publicKeyGeneratePublic, true);
            byte[] bArrGenerateSecret = keyAgreementB.generateSecret();
                        ...
                        while (true) {
                            ...
                            if (!bigIntegerMod3.equals(BigInteger.ZERO)) {
                                ...
                                if (bigIntegerModPow.add(bigIntegerMod4).equals(bigIntegerA)) {
                                    ...
                                    i2++;
                                    if (i2 == 128 && !bigIntegerA.isProbablePrime(80)) {
                                        throw new InvalidAlgorithmParameterException("p is not prime");
                                    }
                                }
                            }
            ...
            if (true != bigInteger2.testBit(0)) {
                bigIntegerA.subtract(bigInteger2).mod(bigIntegerA);
            }
