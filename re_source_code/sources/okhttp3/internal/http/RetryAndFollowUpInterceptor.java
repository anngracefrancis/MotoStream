...
public final class RetryAndFollowUpInterceptor implements Interceptor {
    ...
    private final Request buildRedirectRequest(Response userResponse, String method) {
        ...
        if (!m.a(httpUrlResolve.scheme(), userResponse.request().url().scheme()) && !this.client.followSslRedirects()) {
            return null;
        }
    ...
    private final boolean isRecoverable(IOException e2, boolean requestSendStarted) {
        ...
        return (((e2 instanceof SSLHandshakeException) && (e2.getCause() instanceof CertificateException)) || (e2 instanceof SSLPeerUnverifiedException)) ? false : true;
    }
    ...
    private final int retryAfter(Response userResponse, int defaultDelay) {
        ...
        if (!new Regex("\\d+").b(strHeader$default)) {
            return Integer.MAX_VALUE;
        }
        Integer numValueOf = Integer.valueOf(strHeader$default);
        m.e(numValueOf, "Integer.valueOf(header)");
            ...
            try {
                ...
                try {
                    ...
                    if (!recover(e2, call, request, !(e2 instanceof ConnectionShutdownException))) {
                        throw Util.withSuppressed(e2, listJ);
                    }
                    listJ = c0.p0(listJ, e2);
                    call.exitNetworkInterceptorExchange$okhttp(true);
                    z = false;
                } catch (RouteException e3) {
                    if (!recover(e3.getLastConnectException(), call, request, false)) {
                        throw Util.withSuppressed(e3.getFirstConnectException(), listJ);
                    }
                    listJ = c0.p0(listJ, e3.getFirstConnectException());
                    call.exitNetworkInterceptorExchange$okhttp(true);
                    z = false;
                }
            } catch (Throwable th) {
